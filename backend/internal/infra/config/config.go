...
func (c Config) Validate() error {
	...
	if c.Audit.RetentionCount < -1 || c.Audit.RetentionCount > 1000000000 {
		return errors.New("audit.retentionCount 必须在 -1 到 1000000000 之间")
	}
	...
	return Config{
		...
		Provider: ProviderConfig{
			...
			Console: ConsoleProviderConfig{
				BaseURL: "https://console.x.ai", ChatTimeout: Duration(5 * time.Minute), StreamIdleTimeout: Duration(settingsdomain.DefaultConsoleStreamIdleTimeout),
		...
		Audit: AuditConfig{
			...
			RetentionDays: 7, RetentionCount: -1,
