...
func normalizeRequest(body []byte, spec ModelSpec, configs ...Config) ([]byte, error) {
	return normalizeRequestWithMetadata(body, spec, nil, configs...)
}

func normalizeRequestWithMetadata(body []byte, spec ModelSpec, metadata *provider.NormalizedRequestMetadata, configs ...Config) ([]byte, error) {
...
func mergeSearchTools(payload map[string]any) {
	...
	defaults := []any{
		map[string]any{"type": "web_search", "enable_image_understanding": !hasConsoleFunctionTool(tools, "view_image")},
