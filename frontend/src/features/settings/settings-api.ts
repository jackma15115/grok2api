...
export type SettingsConfigDTO = {
  ...
  audit: { bufferSize: number; batchSize: number; flushInterval: string; commitDelayMS: number; retentionDays?: number; retentionCount?: number };
...
const settingsConfigValidator = hasShape({
  ...
  providerWeb: hasShape({
    ...
    statsigSignerURL: isString, statsigMaterialURL: isString, clearanceMode: isOneOf("manual", "flaresolverr", "on_demand"), flareSolverrURL: isString,
  ...
  audit: hasShape({
    ...
    retentionDays: isOptional(isNumber), retentionCount: isOptional(isNumber),
  }),
  ...
  return {
    ...
    config: {
      ...
      audit: {
        ...
        retentionCount: snapshot.config.audit.retentionCount ?? -1,
      },
      routing: {
