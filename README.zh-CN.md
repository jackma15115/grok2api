...
FlareSolverr 不会暴露到宿主机；每个 Web 或 Console 出口节点均使用自身代理获取匹配的 Cookie 与 User-Agent。
...
`GROK2API_MEMORY_SOFT_LIMIT` 用于设置 Go 运行时的内存软限制，例如：

```bash
GROK2API_MEMORY_SOFT_LIMIT=768MiB docker compose up -d
```

值的格式为“正整数 + 可选单位”。不带单位时按字节解析；支持十进制单位 `B`、`KB`、`MB`、`GB`、`TB`，以及二进制单位 `KiB`、`MiB`、`GiB`、`TiB`，单位不区分大小写。例如 `805306368`、`768MiB`、`1GiB` 和 `2GB` 均有效。不支持 `0.75GiB`、`80%`、`768M` 等格式；非空值格式错误时服务会拒绝启动。

该变量控制 Go 垃圾回收的目标内存预算，不会修改容器的硬内存上限，也不保证进程 RSS 永远低于该值。留空时，Linux 容器若存在 cgroup 内存上限，会自动使用其 80%；未检测到 cgroup 上限时不自动设置。若同时设置标准 Go 环境变量 `GOMEMLIMIT`，则以 `GOMEMLIMIT` 为准，忽略 `GROK2API_MEMORY_SOFT_LIMIT`。
