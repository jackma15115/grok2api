...
| `docker-compose.seed-hex-catch.yml` | SVG seed/HEX collector and edge signer with embedded FlareSolverr | `docker compose -f docker-compose.seed-hex-catch.yml up -d` |
...
`seed-hex-catch` exposes its current browser-captured seed/HEX through both a compatible `/sign` endpoint and a `/material` endpoint. The current pair has no expiry and is atomically replaced after each successful capture.
...
When both containers share a network, either select `URL` mode with `http://seed-hex-catch:8789/sign`, or select `Local` mode with `http://seed-hex-catch:8789/material`. See [seed-hex-catch/README.md](seed-hex-catch/README.md) for configuration details.
...
To automatically maintain Grok Web/Console Cloudflare Clearance, start the optional FlareSolverr Compose service:
...
FlareSolverr is not published on the host; each Web or Console egress node uses its own proxy to obtain cookies and User-Agent.
...
`GROK2API_MEMORY_SOFT_LIMIT` sets the Go runtime's soft memory limit. For example:

```bash
GROK2API_MEMORY_SOFT_LIMIT=768MiB docker compose up -d
```

The value is a positive integer followed by an optional unit. A value without a unit is interpreted as bytes. Decimal units `B`, `KB`, `MB`, `GB`, and `TB`, and binary units `KiB`, `MiB`, `GiB`, and `TiB` are supported case-insensitively. For example, `805306368`, `768MiB`, `1GiB`, and `2GB` are valid. Fractions, percentages, and abbreviated units such as `0.75GiB`, `80%`, or `768M` are not accepted; a non-empty invalid value prevents the service from starting.

This controls the Go garbage collector's target memory budget. It does not change the container's hard memory limit and does not guarantee that process RSS always stays below the value. When left empty, a Linux container with a detectable cgroup memory limit automatically uses 80% of that limit; otherwise no automatic limit is applied. If the standard Go `GOMEMLIMIT` variable is also set, `GOMEMLIMIT` takes precedence and `GROK2API_MEMORY_SOFT_LIMIT` is ignored.
